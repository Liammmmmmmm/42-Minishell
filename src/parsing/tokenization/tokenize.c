...
/*   Updated: 2025/03/10 15:08:50 by lilefebv         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	add_fantom_cmd(t_minishell *minishell)
{
	...
	while (node)
	{
		if (is_redir_t(tget_a(node)) && !is_redir_t(tget_p(node))
			&& tget_p(node) != PAREN_CLOSE
			&& tget_p(node) != COMMAND && tget_p(node) != FILE_R)
		{
